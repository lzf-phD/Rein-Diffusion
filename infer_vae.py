...
def infer(args):
    ...
    im_dataset_cls = {
        'rein': ReinDataset,
    }.get(dataset_config['name']) #依据yaml的任务name选定数据集
    ...
    idxs = torch.randint(0, len(im_dataset) - 1, (num_images,)) #取两个正整数
    ...
    with torch.no_grad():
        ...
        if train_config['save_latents']:
            ...
            part_count = 0
            count = 0
            for idx, im in enumerate(tqdm(data_loader)): #这里才是逐个遍历保存为pkl文件
                _, encoded_output = model.encode(im.float().to(device)) #前面是4通道，后面一个8通道是out[均值+方差]
                fname_latent_map[im_dataset.imageB[idx]] = encoded_output.cpu() #取文件路径作为key,vae潜在编码输出作为value(字典储存)
                # Save latents every 1000 images #每1000张保存一个pkl文件
                if (count + 1) % 1000 == 0:
            ...
            '''
            .pkl（Python 的 pickle）格式非常适合保存这类复杂潜空间的结构化数据
            '''
